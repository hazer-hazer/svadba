<template>
    <div class="card">
        <div class="content">
            <div class="left">
                • 📍 Свадьба состоится 16 августа 2025 года. Место проведения:
                <a class="swiper-no-swiping" style="color: #897; font-weight: bolder;"
                    href="https://yandex.ru/maps/-/CHROaPmo" target="_blank">
                    «Парк-отель Остров
                    Рыболовная усадьба»
                </a>
                <Icon name="lucide:link" style="padding: 0;padding-left: 0;font-size: 0.8rem;" />
                . Сбор гостей в
                14:30.
            </div>
            <div class="api-img left-photo">
                <img src="/api.png">
            </div>
            <div class="us-img right-photo">
                <img src="/us3.png">
            </div>
            <div class="right">
                • 🤵 Мероприятие пройдёт на открытой площадке на природе.
                <br> Для вашего комфорта просим подготовить верхнюю
                одежду по сезону.
            </div>
        </div>
    </div>
</template>

<style scoped lang="scss">
// .api-img {
//     position: absolute;
//     bottom: 10%;
//     left: -5%;
//     width: clamp(5rem, 350px, 42%);
//     max-height: 300px;
//     aspect-ratio: 9/12;
//     border-radius: 1rem;
//     transform: rotateZ(-10deg);
//     box-shadow: 10px 5px 5px -10px #000;
//     border-radius: 1.5rem;

//     img {
//         position: absolute;
//         width: 100%;
//         border-radius: 1.5rem;
//         border: none;
//         object-fit: cover;
//     }
// }

// .us-img {
//     position: absolute;
//     top: -1%;
//     right: -2%;
//     width: clamp(23%, 300px, 40%);
//     aspect-ratio: 9/12;
//     max-height: 300px;
//     // filter: sepia(70%) opacity(0.9);

//     transform: rotateZ(8deg);

//     box-shadow: -15px 15px 25px -20px #000a;

//     border-radius: 25px;
//     overflow: hidden;

//     img {
//         width: 100%;
//         height: 100%;
//         object-fit: cover;
//     }
// }

// .card {
//     .content {
//         .right-block {
//             padding-top: 30px;
//         }
//     }
// }</style>
